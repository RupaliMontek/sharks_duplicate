        ...
        <section class="content">
          <div class="row">
            <div class="col-xs-12">
              <div class="box">
                <div class="box-body">
                 <form method="post" name="EditCandidate" id="EditCandidate"  enctype="multipart/form-data"  action="<?php echo base_url('recruiter/recruiter/update');?>" onsubmit="return validateFP();"> 
                  ...
                  <div class="row">
                     ...
                     <input type="hidden" name="does_email_exists_email_id" id="does_email_exists_email_id" value="1">
                      ...
                      <div class="col-md-3">
                        <input type="text" readonly id="date" class="form-control" name="date" value="<?php if(!empty($result->date)) echo $result->date; ?>" >
                      </div>
...
<option value="Screen reject" <?php if(@$result->final_status=="Screen reject")  echo 'selected'; ?> >Screen Reject</option>
<option value="Need to Reschedule" <?php if(@$result->final_status=="Need to Reschedule")  echo 'selected'; ?> >Need to Reschedule</option>
<option value="Drop" <?php if(@$result->final_status=="Drop")  echo 'selected'; ?> >Drop</option>
                         </select>
                      </div>
                  </div><br>
...
<div class="form-group">
                      <div class="row ppserchflp-pp ppuploadpicp-p">
                        <div class="col-md-2 col-sm-12 col-xs-12">
                            <?php if(!empty($company_images)) { ?>
                                <label for="sel1" class="col-md-6 col-sm-6 col-xs-12 text-center pppostfontp-p">OldPhotos</label>
                            <?php } ?>
                        </div>
                         <div class="col-md-10 col-sm-12 col-xs-12">
                             <?php foreach ($company_images as $row) { ?>
                                  <div class="col-md-2 col-sm-4 col-xs-6 pppht-p">
                                    <!--<img style="width:100px; height:100px;margin-bottom: 10px; display: block;" src="<?php echo base_url();?><?php echo $row->uploaded_file; ?>"/>-->
                                    <a href="<?php echo base_url();?>uploads/recuiter_company_images/<?php echo $result->user_id; ?>/<?php echo $row->uploaded_file; ?>" class="btn btn-warning" target="_blank">View/Download</a>
                                    <a href="javascript:void(0);" class="btn btn-warning" onclick="delenquiry(<?php echo $row->tbl_d_recruiter_id;?>,<?php echo $result->user_id; ?>)">Delete</a>
                                   </div>
                                <?php } ?>
                          </div>
                    </div>
...
<script type="text/javascript">
 ...
 function delenquiry(id,user_id){
    
        var base_url = "<?php echo base_url();?>";
        var picture_id=id;
        var user_id=user_id;
        $.ajax({
          url: base_url+'recruiter/recruiter/delete_basics_company_picture',
          type: 'POST',
          data: {picture_id:picture_id,user_id:user_id},
          success:function(data)
          {
             location.reload();
